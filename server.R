	   ...
	   #datos<- reactive({load("datos.RData")})
		...
		output$newHist <- renderPlot({
		 ...
		 #datos$time <- as.Date(paste(datos$YYYY,datos$mm,datos$dd,sep="-"))
		  ...
		  datos1 <- subset(datos, YYYY>= 2000 & YYYY<=input$mu & lat>=input$latmin & lat <=input$latmax &lon>=input$lonmin & lon<=input$lonmax &datos$prof<300)
		  xrang <- c(seq(floor(min(datos1$mag)),ceiling(max(datos1$mag)),by=1))
